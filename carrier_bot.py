...
async def c_start(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    if c and c['is_verified']:
        await update.message.reply_text("👋 " + c['full_name'] + "!
...
".join(lines), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Принять", callback_data="a_" + str(o['order_id']))]]))
...
async def accept_cb(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    for aid in ADMIN_IDS:
        try:
            await context.bot.send_message(aid, "🚛 #" + str(oid) + " принят перевозчиком " + str(cid) + "!")
        except: pass
...
async def my_orders_c(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    for o in orders:
        ...
        if d:
            ...
            lines.append("⏳ Детали еще не добавлены.")
        lines.append("")
        lines.append("📞 По вопросам к админу.")
        await update.message.reply_text("
".join(lines), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Завершить", callback_data="x_" + str(o['order_id']))]]))
...
async def complete_cb(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    for aid in ADMIN_IDS:
        try:
            await context.bot.send_message(aid, "✅ #" + str(oid) + " выполнен!")
        except: pass
...
async def c_menu_h(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    if t == "📋 Заказы":
        await available_orders(update, context)
        return C_MENU
    elif t == "🚛 Мои заказы":
        await my_orders_c(update, context)
        return C_MENU
    elif t == "📞 Поддержка":
        await c_support(update, context)
        return C_MENU
    elif t == "🔙 Назад":
        await update.message.reply_text("Меню:", reply_markup=c_menu_kb())
        return C_MENU
    else:
        for aid in ADMIN_IDS:
            try:
                await context.bot.send_message(aid, "💬 От перевозчика " + str(update.effective_user.id) + ":" + "
" + t)
            except: pass
