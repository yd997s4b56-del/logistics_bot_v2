...
async def start(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    c = get_customer(uid)
    if c:
        await update.message.reply_text(f"👋 Снова здравствуйте, {c['full_name']}!", reply_markup=menu_kb())
        return MENU
...
async def weight(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Шаг 7/8: Объём (м³):")
    return VOLUME
...
async def price(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    for aid in ADMIN_IDS:
        try:
            await context.bot.send_message(aid, f"🆕 Заявка #{oid}!
📍 {context.user_data['fc']} → {context.user_data['tc']}
📦 {context.user_data['cg']}
💰 {price:,} ₸")
        except: pass
    await update.message.reply_text(f"✅ Заявка #{oid} создана! После принятия перевозчиком вы сможете уточнить детали.", reply_markup=menu_kb())
...
async def my_orders(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    if not orders:
        ...
        return
    for o in orders:
        em = {'new':'🆕','accepted':'✅ Принят','in_progress':'🚛 В пути','completed':'✔️','cancelled':'❌'}.get(o['status'],o['status'])
        txt = f"📦 #{o['order_id']}
...
Статус: {em}
"
        if o['carrier_id']: txt += "🚛 Перевозчик назначен
"
        kb = []
        if o['status'] in ('accepted','in_progress'):
            kb.append([InlineKeyboardButton("➕ Добавить детали", callback_data=f"d_{o['order_id']}")])
        kb.append([InlineKeyboardButton("📋 Детали", callback_data=f"v_{o['order_id']}")])
...
async def d_type(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"Введите детали для '{context.user_data['dt']}':")
    return D_VAL
...
async def d_val(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    for aid in ADMIN_IDS:
        try: await context.bot.send_message(aid, f"📝 Детали к #{oid}
{context.user_data['dt']}: {update.message.text}")
    ...
    context.user_data.pop('d_oid',None); context.user_data.pop('dt',None)
    return MENU

async def view_cb(update:Update, context:ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    oid = int(q.data.split('_')[1])
    o = get_order(oid); d = get_details(oid)
    txt = f"📦 #{oid}
🚚 {o['from_city']}, {o['from_address']}
🏁 {o['to_city']}, {o['to_address']}
...
async def menu_h(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    if t == "📝 Новая заявка":
        await update.message.reply_text("📝 Шаг 1/8: Город отправления:")
        return F_CITY
    elif t == "📋 Мои заявки": await my_orders(update, context); return MENU
    elif t == "📞 Поддержка": await support(update, context); return MENU
    elif t == "🔙 Назад": await update.message.reply_text("Меню:", reply_markup=menu_kb()); return MENU
    else:
        for aid in ADMIN_IDS:
            try: await context.bot.send_message(aid, f"💬 От заказчика {update.effective_user.id}:
{t}")
            except: pass
